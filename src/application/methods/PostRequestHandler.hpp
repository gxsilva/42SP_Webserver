...
class PostRequestHandler
{
	...
	private:
		...
		bool				 extractUploadFileName(const std::string& uri, std::string& uploadFileName) const;
		bool				 isSafeUploadFileName(const std::string& fileName) const;
		std::string			 buildUploadTarget(const std::string& filePath, const std::string& uploadFileName) const;
		...
		HttpResponse		 writeRequestBody(const HttpRequest& request, const std::string& targetPath,
