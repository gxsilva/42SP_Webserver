...
enum HttpStatusCode
{
	...
	*/
	...
	CREATED =
		201, // O pedido funcionou e resultou na criação de um novo recurso (comum em métodos POST).
	ACCEPTED = 202, // O servidor aceitou o pedido, mas ele ainda está sendo processado (comum em
					// tarefas em segundo plano).
	NO_CONTENT = 204, // Sucesso total, mas o servidor não tem nada para te enviar de volta no corpo
					  // da mensagem.
	...
	*/
	MOVED_PERMANENTLY =
		301, // O endereço mudou para sempre. O navegador deve usar a nova URL daqui para frente.
	FOUND = 302, // O recurso está em outro lugar agora, mas é temporário. Continue usando a URL
				 // original no futuro.
	NOT_MODIFIED = 304, // O recurso não mudou desde a última vez que você o baixou. Use o que está
						// no seu cache para economizar banda.
	...
	TOO_MANY_REQUESTS = 429, // Você está enviando requisições rápido demais. O servidor está te
							 // bloqueando temporariamente (Rate Limit).
	...
	*/
	INTERNAL_SERVER_ERROR = 500, // O "erro genérico". Algo quebrou no código do servidor e ele não
								 // sabe explicar exatamente o quê.
	NOT_IMPLEMENTED =
		501, // O servidor não suporta a funcionalidade necessária para completar a requisição.
	SERVICE_UNAVAILABLE =
		503, // O servidor está sobrecarregado ou em manutenção. Tente novamente mais tarde.
	HTTP_VERSION_NOT_SUPPORTED =
		505 // O servidor não suporta a versão do protocolo HTTP que você usou na requisição.
