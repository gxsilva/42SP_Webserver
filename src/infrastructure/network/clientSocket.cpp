...
ClientSocket::~ClientSocket() { invalidate(); }

void ClientSocket::invalidate() { _fd.invalidate(); }

bool ClientSocket::isValid() const { return (_fd.isValid()); }

int ClientSocket::getPollFd() const { return (_fd.get()); }
...
void ClientSocket::appendToWriteBuffer(const char* data, size_t len) { _writeBuffer.append(data, len); }

bool ClientSocket::hasDataToSend() const { return (!_writeBuffer.empty()); }
