...
#include "../src/infrastructure/io/request/HttpRequestParser.hpp"
#include <cassert>
#include <cstdio>
#include <cstring>
#include <iostream>
...
class TestPostWithBody : public TestCase
{
	public:
		...
		void run()
		{
			...
			assertTrue(req.getHeaders().find("CONTENT-LENGTH") != req.getHeaders().end(),
					   "Content-Length header should exist");
		}
};
...
class TestLFOnlyLineEndings : public TestCase
{
	public:
		...
		void run()
		{
			...
			assertTrue(req.getHeaders().find("HOST") != req.getHeaders().end(), "Host header should exist");
			assertTrue(req.getHeaders().find("CONTENT-TYPE") != req.getHeaders().end(),
...
class TestDeleteRequest : public TestCase
{
	public:
		...
		void run()
		{
			...
			assertTrue(req.getHeaders().find("AUTHORIZATION") != req.getHeaders().end(),
...
class TestPostWithJsonBody : public TestCase
{
	public:
		...
		void run()
		{
			...
			assertTrue(req.getHeaders().find("CONTENT-TYPE") != req.getHeaders().end(),
					   "Content-Type header should exist");
			assertEqual(req.getHeaders().at("CONTENT-TYPE"), "application/json",
...
int main()
{
	...
	TestCase* tests[] = {
		new TestHttpRequestConstructor(), new TestSimpleGetRequest(),	  new TestPostWithBody(),
		new TestMultipleHeaders(),		  new TestHeaderSpacesTrimming(), new TestLFOnlyLineEndings(),
		new TestDeleteRequest(),		  new TestPostEmptyBody(),		  new TestRequestWithQueryString(),
		new TestPostWithJsonBody(),		  new TestHttp10Version(),		  new TestHeaderCaseSensitivity()};
